...
class exp_loss(t.nn.Module): 
    ...
    def forward(self,x,K):
        return t.exp(self.scale * (x + 1./(K-1)))-1
...
class doubleexp_loss(t.nn.Module): 
    ...
    def forward(self,x,K):
        return t.exp(self.scale * (x+1./(K-1)))+t.exp(-self.scale * (x + 1./(K-1)))-2
...
class nudging_model(t.nn.Module):
    ...
    def forward(self, initial_prototypes, K=100):
        ...
        prod_sim_loss = self.act(prod_sims, K)
        ...
        total_loss = prod_sim_loss + 0.1*avg_deviation_loss
